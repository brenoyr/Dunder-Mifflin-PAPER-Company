...
def fat32():
    ...
    for i in range(13):
        ...
        print "Where file starts: {}".format(offset)
        ...
        for c in cluster_addr_list:
            # print "this is the last index before crashing: {}".format(dataSectionAddr+int(c,16))
            # print "c: {}".format(c)
            # print "c (dec): {}".format(int(c,16))
            # print "len of cluster addr: {}".format(len(cluster_addr_list))
            # print "last cluster addr:: {}".format(cluster_addr_list[-1])
            # f.write(hex_list[dataSectionAddr+int(c,16)])        ################# playing with this
            f.write(hex_list[dataSectionAddr+int(c,16)])        ################# playing with this
        f.close() 
